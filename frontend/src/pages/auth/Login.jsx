...
const Login = ({ onLoginSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Aquí se guarda correctamente el ID para que el formulario de adopción lo lea
      sessionStorage.setItem("userId", user.id);

      const rol = (user.rol || "ADOPTANTE").toUpperCase();
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-6 bg-gradient-to-br from-orange-50 via-white to-orange-100">
      <div className="w-full max-w-md bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-white p-10">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <button
            type="submit"
            className="w-full mt-4 py-4 rounded-2xl bg-gradient-to-r from-orange-500 to-orange-600 text-white font-bold text-lg shadow-lg hover:shadow-xl hover:scale-[1.02] transition duration-300"
          >
            Ingresar
          </button>
        </form>
